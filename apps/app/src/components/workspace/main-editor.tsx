...
import { combineVariables, extractVariablesWithMetadata, type ParsedJinjaVariable } from "@/lib/jinja-parser"
...
export function MainEditor() {
  ...
  const [detectedVariables, setDetectedVariables] = useState<ParsedJinjaVariable[]>([])
  ...
  const systemEditorRef = useRef<monaco.editor.IStandaloneCodeEditor | null>(null)
  const userEditorRef = useRef<monaco.editor.IStandaloneCodeEditor | null>(null)
  ...
  const validateJinja2Syntax = (
    ...
    lines.forEach((line, lineIndex) => {
      ...
      for (const match of variableMatches) {
        ...
        if (variableName && !detectedVariables.some(v => v.name === variableName)) {
          ...
          if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(variableName)) {
            // validationErrors.push({ // This line was removed from the new_code, so it's removed here.
            //   line: lineNumber,
            //   message: `Invalid variable name "${variableName}": must start with letter or underscore, followed by letters, numbers, or underscores`,
            // });
          }
        }
      }
  ...
  const handleVariableValuesChange = (values: Record<string, string | number>) => {
  ...
  useEffect(() => {
    ...
    if (detectedVariables.length > 0 && Object.keys(variableValues).length === 0) {
      ...
      const defaultValues = detectedVariables.reduce(
        (acc, variable) => {
      ...
      provideCompletionItems: (model, position) => {
        ...
        for (const variable of detectedVariables) {
          suggestions.push({
            ...
            detail: variable.description || "Template variable",
            range: {
              startLineNumber: position.lineNumber,
    ...
    <div className='h-full flex flex-col'>
            ...
            <div className='h-[200px]'>
              <Editor
                ...
                options={{
                  ...
                  placeholder: "Enter your system prompt with Jinja2 variables..."
                }}
              />
            </div>
          </CardContent>
            ...
            <div className='h-[200px]'>
              <Editor
                ...
                options={{
                  ...
                  suggestOnTriggerCharacters: true,
                  quickSuggestions: true,
                  placeholder: "Enter your user prompt with Jinja2 variables..."
                }}
              />
            </div>
          </CardContent>
      ...
      <div className='border-t border-border'>
        <Button
          ...
          {isVariablePanelExpanded ? <ChevronUp className='w-4 h-4' /> : <ChevronDown className='w-4 h-4' />}
        </Button>

        {isVariablePanelExpanded && (
          <div className='p-4 border-t border-border bg-muted/20'>
            <VariableForm variables={detectedVariables} onSubmit={handleVariableValuesChange} />
          </div>
        )}
      </div>
    </div>
