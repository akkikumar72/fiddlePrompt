...
import { generateVariablesSchema, generateDefaultValues } from "@/lib/variable-validation"
...
export default function VariableForm({ variables, onSubmit, onValuesChange, className = "" }: VariableFormProps) {
  ...
  const lastValuesRef = useRef<Record<string, string | number>>({})
  ...
  useEffect(() => {
    ...
    const varsChanged =
      ...
      variables.some(
        (v, i) => !prevVariables[i] || prevVariables[i].name !== v.name || prevVariables[i].type !== v.type
      )
    ...
    for (const [key, value] of Object.entries(lastValuesRef.current || {})) {
      if (Object.hasOwn(newDefaults, key) && value !== undefined && value !== null && value !== "") {
            ...
            {variable.type === "boolean" ? (
              <div className='flex items-center space-x-2'>
                <Controller
                  ...
                  control={control}
                  render={({ field }) => (
                    <Switch id={variable.name} checked={field.value} onCheckedChange={field.onChange} />
                  )}
                />
                <Label htmlFor={variable.name} className='text-sm'>
              ...
              <Input
                ...
                {...register(variable.name)}
                placeholder={`Enter value for ${variable.name}`}
                className={`w-full ${errors[variable.name] ? "border-red-400 focus:ring-red-400" : ""}`}
              />
            )}

            {errors[variable.name] && (
              <p className='text-xs text-red-500 mt-1'>{String(errors[variable.name]?.message)}</p>
            )}
          </div>
        ))}
      </div>
