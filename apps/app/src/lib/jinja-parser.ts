...
export function extractVariablesWithMetadata(text: string): ParsedJinjaVariable[] {
  ...
  for (const match of matches) {
    ...
    // Parse validation metadata if present
    if (validationStr) {
      ...
      for (const validation of validations) {
        ...
        if (key === "min") {
          variable.minLength = Number.parseInt(value)
        } else if (key === "max") {
          variable.maxLength = Number.parseInt(value)
        } else if (key === "default") {
  ...
  const deduplicated = variables.reduce<Record<string, ParsedJinjaVariable>>((acc, variable) => {
...
export function combineVariables(texts: string[]): ParsedJinjaVariable[] {
  ...
  return Object.values(
    allVariables.reduce<Record<string, ParsedJinjaVariable>>((acc, variable) => {
      const existingVar = acc[variable.name]
      if (
        !existingVar ||
        ((!existingVar.type || !existingVar.defaultValue) && (variable.type || variable.defaultValue))
      ) {
        acc[variable.name] = variable
      }
      return acc
